...
class LoggerManager:
    ...
    def __init__(self, logger_name, log_level=None):
        ...
        self.logger.info(
            'creating an instance of the Logger {}'.format(logger_name))
...
def log_function_runtime(exec_time_record=None):
    def wrapper(orig_func):
        func_runtime_logger = LoggerManager("InstallationAutomation").get_logger()
...
def log_enter_and_exit(orig_func):
    logger = LoggerManager("InstallationAutomation").get_logger()
    @wraps(orig_func)
